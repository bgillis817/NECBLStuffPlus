...
prepare_location_data <- function(df) {

  cat("NECBL Location+ Data Preparation \n")
  cat("================================================\n")
...
build_location_plus_model <- function(df) {

  cat("\n\nBuilding NECBL Location+ Model \n")
  cat("=====================================================\n\n")
  ...
  model_formula <- as.formula("PitchCall ~ PlateLocHeight + PlateLocSide + Balls + Strikes + PitcherThrows + BatterSide")
...
calculate_location_plus <- function(model_results) {
  ...
  pitcher_summary <- pitcher_summary %>%
    mutate(
      ...
      location_plus = round(100 - (z_score * 10), 1),
      percentile    = round(percent_rank(location_plus) * 100, 1)
    ) %>%
    left_join(pitcher_names, by = c("PitcherId", "PitcherTeam", "Season")) %>%
    select(PitcherId, Pitcher, PitcherTeam, Season, n_pitches, location_plus, percentile) %>%
    arrange(desc(location_plus))

  cat(sprintf("Mean: %.1f  SD: %.1f\n", mean(pitcher_summary$location_plus), sd(pitcher_summary$location_plus)))
  return(pitcher_summary)
}


calculate_location_plus_by_pitch <- function(model_results) {

  cat("\n\nCalculating Location+ by Pitch Type\n")
  cat("=================================================\n\n")
  ...
  pitch_summary <- full_data %>%
    ...
    left_join(pitcher_names, by = c("PitcherId", "PitcherTeam", "Season")) %>%
    select(PitcherId, Pitcher, PitcherTeam, TaggedPitchType, Season, n_pitches, location_plus, percentile) %>%
...
run_necbl_location_plus_no_threshold <- function() {
  ...
  cat("\nTop 10 Overall:\n")
  print(head(overall_summary %>% arrange(desc(location_plus)), 10))

  return(list(overall = overall_summary, by_pitch = pitch_summary, model = model_results$model))
}


results <- run_necbl_location_plus_no_threshold()
