...
get_latest_github_filename <- function(pattern) {
  ...
  files <- httr::content(resp, as = "parsed")
  names_vec <- sapply(files, function(f) f$name)
  matches <- sort(grep(pattern, names_vec, value = TRUE))
...
load_all_data <- function() {
  ...
  data_list <- list(
    pitching_overall = if (!is.null(f_pitching_overall))
      load_github_data(paste0(base_url, f_pitching_overall), "Pitching+ Overall") else NULL,
    stuff_overall = if (!is.null(f_stuff_overall))
      load_github_data(paste0(base_url, f_stuff_overall), "Stuff+ Overall") else NULL,
    location_overall = if (!is.null(f_location_overall))
      load_github_data(paste0(base_url, f_location_overall), "Location+ Overall") else NULL,
    pitching_pitch = if (!is.null(f_pitching_pitch))
      load_github_data(paste0(base_url, f_pitching_pitch), "Pitching+ by Pitch") else NULL,
    stuff_pitch = if (!is.null(f_stuff_pitch))
      load_github_data(paste0(base_url, f_stuff_pitch), "Stuff+ by Pitch") else NULL,
    location_pitch = if (!is.null(f_location_pitch))
...
ui <- dashboardPage(
  ...
  dashboardBody(
    ...
    tabItems(
      ...
      tabItem(tabName = "overall_leaders",
        fluidRow(
          column(4, box(title = "Stuff+ Leaders",    status = "success", solidHeader = TRUE, width = NULL, DT::dataTableOutput("stuff_leaders_table"))),
          column(4, box(title = "Location+ Leaders", status = "warning", solidHeader = TRUE, width = NULL, DT::dataTableOutput("location_leaders_table"))),
          column(4, box(title = "Pitching+ Leaders", status = "danger",  solidHeader = TRUE, width = NULL, DT::dataTableOutput("pitching_leaders_table")))
        )
      ),

      tabItem(tabName = "complete_rankings",
        fluidRow(box(title = "Complete Pitcher Rankings - All Metrics", status = "primary", solidHeader = TRUE, width = 12, DT::dataTableOutput("complete_rankings_table"))),
        fluidRow(box(title = "Metrics Distribution", status = "primary", solidHeader = TRUE, width = 12, plotlyOutput("distribution_plot", height = "400px")))
      ),

      tabItem(tabName = "pitch_analysis",
        fluidRow(box(title = "Select Pitch Type", status = "primary", solidHeader = TRUE, width = 12,
          selectInput("pitch_type_select", "Pitch Type:",
                      choices = c("Fastball","Slider","Curveball","Changeup","Cutter","Sinker","Sweeper"),
                      selected = "Fastball"))),
        fluidRow(
          column(4, box(title = "Stuff+ by Pitch Type",    status = "success", solidHeader = TRUE, width = NULL, DT::dataTableOutput("stuff_pitch_type_table"))),
          column(4, box(title = "Location+ by Pitch Type", status = "warning", solidHeader = TRUE, width = NULL, DT::dataTableOutput("location_pitch_type_table"))),
          column(4, box(title = "Pitching+ by Pitch Type", status = "danger",  solidHeader = TRUE, width = NULL, DT::dataTableOutput("pitching_pitch_type_table")))
        )
      ),

      tabItem(tabName = "pitch_complete",
        fluidRow(box(title = "Complete Pitch Type Board - All Pitchers, All Pitch Types, All Metrics",
          status = "primary", solidHeader = TRUE, width = 12,
          column(12, p("This table shows every pitcher's metrics for each pitch type they throw."), br(),
                 DT::dataTableOutput("pitch_complete_board"))))
      ),

      tabItem(tabName = "pitch_rankings",
        fluidRow(box(title = "Select Pitch Type for Complete Rankings", status = "primary", solidHeader = TRUE, width = 12,
          selectInput("pitch_type_complete", "Pitch Type:",
                      choices = c("Fastball","Slider","Curveball","Changeup","Cutter","Sinker","Sweeper"),
                      selected = "Curveball"))),
        fluidRow(box(title = "Complete Pitch Type Rankings",   status = "primary", solidHeader = TRUE, width = 12, DT::dataTableOutput("complete_pitch_rankings_table"))),
        fluidRow(box(title = "Pitch Type Metrics Comparison",  status = "primary", solidHeader = TRUE, width = 12, plotlyOutput("pitch_comparison_plot", height = "400px")))
      ),

      tabItem(tabName = "search",
        fluidRow(box(title = "Player Search", status = "primary", solidHeader = TRUE, width = 12,
          fluidRow(
            column(6, textInput("player_search", "Search for Player:", placeholder = "Enter player name...")),
            column(6, actionButton("search_button", "Search", class = "btn-primary", style = "margin-top: 25px;"))
          ))),
        fluidRow(box(title = "Player Details",      status = "primary", solidHeader = TRUE, width = 12, uiOutput("player_details"))),
        fluidRow(box(title = "Player Pitch Arsenal", status = "primary", solidHeader = TRUE, width = 12, DT::dataTableOutput("player_arsenal")))
      )
    )
  )
)
...
server <- function(input, output, session) {
  ...
  observe({
    ...
    if (!is.null(data$pitching_overall) && "PitcherTeam" %in% names(data$pitching_overall)) {
      teams <- sort(unique(data$pitching_overall$PitcherTeam[!is.na(data$pitching_overall$PitcherTeam)]))
      updateSelectInput(session, "team_filter", choices = c("All Teams" = "All", teams), selected = "All")
    }
  })

  color_breaks <- c(70, 75, 80, 85, 90, 95, 100, 105, 110, 115, 120, 125)
  ...
  output$stuff_leaders_table <- DT::renderDataTable({
    ...
    filter_data(data$stuff_overall) %>% arrange(desc(stuff_plus)) %>% head(10) %>%
      ...
      formatStyle("stuff_plus", backgroundColor = styleInterval(color_breaks, color_vals))
  })
  ...
  output$location_leaders_table <- DT::renderDataTable({
    ...
    filter_data(data$location_overall) %>% arrange(desc(location_plus)) %>% head(10) %>%
      ...
      formatStyle("location_plus", backgroundColor = styleInterval(color_breaks, color_vals))
  })
  ...
  output$pitching_leaders_table <- DT::renderDataTable({
    ...
    filter_data(data$pitching_overall) %>% arrange(desc(pitching_plus)) %>% head(10) %>%
      ...
      formatStyle("pitching_plus", backgroundColor = styleInterval(color_breaks, color_vals))
  })
  ...
  output$complete_rankings_table <- DT::renderDataTable({
    ...
    pitching_f %>%
      ...
      datatable(colnames = c("Pitcher","Team","Stuff+","Location+","Pitching+"),
                options = list(pageLength=25, scrollY="600px", scrollCollapse=TRUE, dom='Bfrtip'), rownames=FALSE) %>%
      formatStyle(columns = c("stuff_plus","location_plus","pitching_plus"),
                  backgroundColor = styleInterval(color_breaks, color_vals))
  })
  ...
  output$complete_pitch_rankings_table <- DT::renderDataTable({
    ...
    pitching_f %>%
      ...
      arrange(desc(pitching_plus)) %>%
      datatable(colnames = c("Pitcher","Team","Stuff+","Location+","Pitching+"),
                options = list(pageLength=25, scrollY="500px", scrollCollapse=TRUE, dom='Bfrtip'), rownames=FALSE) %>%
      formatStyle(columns = c("stuff_plus","location_plus","pitching_plus"),
                  backgroundColor = styleInterval(color_breaks, color_vals))
  })
  ...
  output$pitch_complete_board <- DT::renderDataTable({
    ...
    pitching_f %>%
      ...
      datatable(colnames = c("Pitcher","Pitch Type","Team","Stuff+","Location+","Pitching+"),
                options = list(pageLength=50, scrollY="600px", scrollCollapse=TRUE, dom='Bfrtip',
  ...
  output$stuff_pitch_type_table <- DT::renderDataTable({
    ...
    filter_data(data$stuff_pitch) %>% filter(TaggedPitchType == input$pitch_type_select) %>%
      ...
      mutate(Team = ifelse("PitcherTeam" %in% names(.) & !is.na(PitcherTeam), PitcherTeam, "N/A")) %>%
      select(Pitcher, stuff_plus, Team) %>%
      datatable(colnames = c("Pitcher","Stuff+","Team"), options = list(pageLength=15, dom='t'), rownames=FALSE) %>%
      formatStyle("stuff_plus", backgroundColor = styleInterval(color_breaks, color_vals))
  })
  ...
  output$location_pitch_type_table <- DT::renderDataTable({
    ...
    filter_data(data$location_pitch) %>% filter(TaggedPitchType == input$pitch_type_select) %>%
      ...
      mutate(Team = ifelse("PitcherTeam" %in% names(.) & !is.na(PitcherTeam), PitcherTeam, "N/A")) %>%
      select(Pitcher, location_plus, Team) %>%
      datatable(colnames = c("Pitcher","Location+","Team"), options = list(pageLength=15, dom='t'), rownames=FALSE) %>%
      formatStyle("location_plus", backgroundColor = styleInterval(color_breaks, color_vals))
  })
  ...
  output$pitching_pitch_type_table <- DT::renderDataTable({
    ...
    filter_data(data$pitching_pitch) %>% filter(TaggedPitchType == input$pitch_type_select) %>%
      ...
      mutate(Team = ifelse("PitcherTeam" %in% names(.) & !is.na(PitcherTeam), PitcherTeam, "N/A")) %>%
      select(Pitcher, pitching_plus, Team) %>%
      datatable(colnames = c("Pitcher","Pitching+","Team"), options = list(pageLength=15, dom='t'), rownames=FALSE) %>%
      formatStyle("pitching_plus", backgroundColor = styleInterval(color_breaks, color_vals))
  })
  ...
  output$distribution_plot <- renderPlotly({
    ...
    plot_ly(combined, x = ~pitching_plus, color = ~Metric, type = "histogram",
      ...
      layout(title = paste("Metrics Distribution -", input$season_filter),
             xaxis = list(title = "Score"), yaxis = list(title = "Count"), barmode = "overlay")
  })
  ...
  output$pitch_comparison_plot <- renderPlotly({
    ...
    if (nrow(filtered_data) > 0) {
      plot_ly(filtered_data, x = ~pitching_plus, type = "histogram",
        ...
        layout(title = paste(input$pitch_type_complete, "Pitching+ Distribution -", input$season_filter),
               xaxis = list(title = "Pitching+ Score"), yaxis = list(title = "Count"))
    }
  })

  observeEvent(input$search_button, {
    data <- all_data(); if (is.null(data$pitching_overall)) return()
    search_term <- tolower(input$player_search)
    matching_players <- data$pitching_overall %>%
      filter(grepl(search_term, tolower(Pitcher)), Season == input$season_filter)
    if (nrow(matching_players) > 0) {
      player_name <- matching_players$Pitcher[1]
      output$player_details <- renderUI({
        pitching_m  <- data$pitching_overall %>% filter(Pitcher == player_name, Season == input$season_filter)
        stuff_m     <- data$stuff_overall    %>% filter(Pitcher == player_name, Season == input$season_filter)
        location_m  <- data$location_overall %>% filter(Pitcher == player_name, Season == input$season_filter)
        team_info <- if ("PitcherTeam" %in% names(pitching_m) && nrow(pitching_m) > 0)
          tags$p(paste("Team:", pitching_m$PitcherTeam[1])) else NULL
        tags$div(
          h4(player_name),
          tags$p(paste("Season:", input$season_filter)),
          team_info,
          tags$p(paste("Stuff+:",    ifelse(nrow(stuff_m)    > 0, round(stuff_m$stuff_plus[1],       1), "N/A"))),
          tags$p(paste("Location+:", ifelse(nrow(location_m) > 0, round(location_m$location_plus[1], 1), "N/A"))),
          tags$p(paste("Pitching+:", ifelse(nrow(pitching_m) > 0, round(pitching_m$pitching_plus[1], 1), "N/A")))
        )
      })
      output$player_arsenal <- DT::renderDataTable({
        arsenal    <- data$pitching_pitch %>% filter(Pitcher == player_name, Season == input$season_filter) %>% select(TaggedPitchType, n_pitches, pitching_plus, any_of("PitcherTeam"))
        stuff_a    <- data$stuff_pitch    %>% filter(Pitcher == player_name, Season == input$season_filter) %>% select(TaggedPitchType, stuff_plus)
        location_a <- data$location_pitch %>% filter(Pitcher == player_name, Season == input$season_filter) %>% select(TaggedPitchType, location_plus)
        arsenal %>%
          left_join(stuff_a,    by = "TaggedPitchType") %>%
          left_join(location_a, by = "TaggedPitchType") %>%
          mutate(Team = ifelse("PitcherTeam" %in% names(.) & !is.na(PitcherTeam), PitcherTeam, "N/A")) %>%
          select(TaggedPitchType, Team, stuff_plus, location_plus, pitching_plus) %>%
          datatable(colnames = c("Pitch Type","Team","Stuff+","Location+","Pitching+"),
                    options = list(pageLength=10), rownames=FALSE) %>%
          formatStyle(columns = c("stuff_plus","location_plus","pitching_plus"),
                      backgroundColor = styleInterval(color_breaks, color_vals))
      })
    } else {
      output$player_details <- renderUI({ tags$p("No player found matching your search.") })
    }
  })
  ...
  observe({
    ...
    if (!is.null(data$pitching_pitch)) {
      pitch_types <- sort(unique(c(data$pitching_pitch$TaggedPitchType[!is.na(data$pitching_pitch$TaggedPitchType)], "Sweeper")))
      updateSelectInput(session, "pitch_type_complete", choices = pitch_types,
                        selected = if ("Curveball" %in% pitch_types) "Curveball" else pitch_types[1])
      updateSelectInput(session, "pitch_type_select",   choices = pitch_types,
                        selected = if ("Fastball"  %in% pitch_types) "Fastball"  else pitch_types[1])
    }
  })
}
